  ...
  <div class="mx-auto max-w-6xl px-4 py-10">
    <div class="grid gap-8 lg:grid-cols-[1fr_20rem]">
      <main class="min-w-0">
        ...
        <section v-if="currentExercise.tools?.length" class="mt-4" :aria-label="t('exercise.toolsUsed')">
          <h2 class="text-xs font-semibold uppercase tracking-wide muted-text">{{ t('exercise.toolsUsed') }}</h2>
          <ul class="mt-2 flex flex-wrap gap-1.5">
            <li v-for="tool in currentExercise.tools" :key="tool.id" class="course-tool-badge rounded-full border border-ink-200 px-2 py-0.5 text-[11px] dark:border-ink-700">
      ...
      <aside class="flex flex-col gap-4">
        ...
        <a v-if="youtubeUrl" :href="youtubeUrl" target="_blank" rel="noopener noreferrer" class="course-subtle surface-card px-4 py-3 text-center text-sm font-semibold hover:bg-ink-50 dark:hover:bg-ink-800/60" @click="onYoutube">{{ t('exercise.watchOnYoutube') }}</a>
        ...
        <NuxtLink :to="localePath(`/courses/${currentCourse.slug}`)" class="course-subtle surface-card px-4 py-3 text-center text-sm font-semibold hover:bg-ink-50 dark:hover:bg-ink-800/60">{{ t('exercise.backToCourse') }}</NuxtLink>
      </aside>
    </div>
  </div>
</template>
